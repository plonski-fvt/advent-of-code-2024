...
def count_reachable_9s(pos: tuple[int, int], 
                       already_queued: set[tuple[int, int]]) -> int:
    ...
    for position in neighbor_positions:
        if position in already_queued:
            continue
        if (position[0] < 0) or (position[0] >= rows) or (position[1] < 0) or (position[1] >= cols):
            continue
        if map[position[0]][position[1]] == pos_number + 1:
            already_queued.add(position)
            reachable_sum += count_reachable_9s(position, already_queued)
...
for i in range(rows):
    for j in range(cols):
        if map[i][j] == 0:
            reachable_sum = count_reachable_9s((i, j), set())
            print(reachable_sum)
